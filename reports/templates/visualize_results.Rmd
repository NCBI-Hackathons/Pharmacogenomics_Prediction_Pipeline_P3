...
median_feature_importance <- apply(feature_importance, 1, median)
...
max_feature_importance <- apply(feature_importance, 1, max)
...
top_n <- apply(feature_importance, 1, function(x) { mean(head(sort(x, decreasing=TRUE), n)) })
...
dist_matrix <- cor(feature_importance, method='spearman')
