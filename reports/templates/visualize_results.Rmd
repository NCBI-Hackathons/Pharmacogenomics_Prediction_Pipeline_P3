...
drugs_to_include <- rownames(rsquared)[apply(rsquared, 1, max, na.rm=TRUE) >= 0.25]
...
write.table(highest_trimmed_mean_feature_importance, sep='\t', quote=FALSE,
            file=file.path(output_dir, 'highest_trimmed_mean_feature_importance.tab'))
```
...
top_drugs <- names(head(sort(apply(rsquared %>% select(-max_rsquared), 1, trimmed_mean), 
                   decreasing=TRUE), 5))
