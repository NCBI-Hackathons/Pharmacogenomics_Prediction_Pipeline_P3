...
summary_df <- cbind(min=apply(rel_risks, 2, min), 
                    median=apply(rel_risks, 2, median),
                    max=apply(rel_risks, 2, max))
kable(summary_df, caption='Relative CV risk by algorithm')

long_df <- melt(rel_risks)
colnames(long_df) <- c('drug', 'learner', 'value')

ggplot(long_df, aes(x=value, fill=learner)) + 
