...
drugDat_sub <- drugDat[which(DrugRange > 1), ]
...
colnames(tdrugDat_sub) <- drugDat_sub[, 1] # use SID as name
...
### loop over compounds and estimate predictor
...
for(ii in 2:10) {
	...
	save(out_SL, file = file.path(paste0(regression_ouput_dir, "/outSL_", colnames(tdrugDat_sub)[ii], ".RData")))
	# print(colnames(tdrugDat_sub)[ii])
	print(out_SL)
	# cat(ii, "\n")
	rm(out_SL)
	gc()
